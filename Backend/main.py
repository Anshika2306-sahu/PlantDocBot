...
@app.get("/")
def root():
    return {"message": "Welcome to PlantDocBot API 🌿"}
...
async def predict_text(symptoms: str = Form(...)):
    ...
    try:
        ...
        confidence = raw_prediction["score"] * 100
        label_str = raw_prediction["label"].upper()

        text = symptoms.lower()

        # Rule-based mapping
        if "brown spots" in text or "yellow leaves" in text:
            disease = "Tomato, Early Blight"
        elif "dark patches" in text or "white mold" in text:
            disease = "Tomato, Late Blight"
        elif "scabby" in text or "velvety" in text:
            disease = "Apple, Apple Scab"
        elif "tan spots" in text or "reddish brown" in text:
            disease = "Corn, Leaf Blight"
        elif "white fungal" in text or "dark lesions" in text:
        ...
        print(f"[Backend] Text Prediction: {disease} ({confidence:.2f}%)")
