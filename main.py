...
# Build image model and load weights (use the filename that exists in your models folder)
cnn = PlantDiseaseModel(num_classes=38)
image_model_path = BASE_DIR / "models" / "ImageClassification_model_weights.pth"  # update if your file is named differently
...
# -------------------------
# Path to local text model directory (inside models/)
text_model_path = BASE_DIR / "models" / "text_classifier_model"

try:
    # IMPORTANT: pass a string path to avoid HF treating it as a repo id
    tokenizer = AutoTokenizer.from_pretrained(str(text_model_path), local_files_only=True)
    text_model = AutoModelForSequenceClassification.from_pretrained(str(text_model_path), local_files_only=True).to(device)
    text_model.eval()
except Exception as e:
