...
const UploadForm = ({ setDiagnosisResult, diagnosisResult }) => {
    ...
    const handleFileChange = (event) => {
        ...
        // --- KEY FIX FOR IMAGE PREVIEW ---
        // 1. Revoke previous object URL to avoid memory leaks
        if (previewUrl) {
            URL.revokeObjectURL(previewUrl); 
        }
        
        // 2. Create a new object URL for the selected file
        if (selectedFile) {
    ...
    // Simulate the API call to your FastAPI backend
    const handleSubmit = async () => {
        if (!file) {
            // Replaced alert() with a console message/UI feedback placeholder
        ...
        const mockConfidence = Math.round(Math.random() * 20 + 75); // 75-95%
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="h-64 mb-4 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden relative">
                {previewUrl ? (
                    // This is where the preview URL is used to display the image
            ...
            <label className="w-full cursor-pointer bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-4 rounded-lg shadow-md transition duration-300 flex items-center justify-center mb-4">
                ...
                {/* Shows file name or default text */}
...
const ChatBox = ({ diagnosisResult }) => {
    ...
    // Convert disease name format (e.g., Tomato___Early_blight) to readable text
...
export default function App() {
    // State to hold and pass diagnosis results between components
    const [diagnosisResult, setDiagnosisResult] = useState(initialDiagnosis);

    return (
        // Main container uses a light background color
        <div className="min-h-screen bg-[#f7fafc] font-sans"> 
          
            {/* === 1. Hero Section (AI Powered Disease Detection) === */}
            <section 
            ...
            >
                {/* Dark green overlay with transparency */}
            ...
            >
                <div className="max-w-6xl mx-auto px-4 text-center">
                    ...
                    {/* Application UI (Upload Form and Chat Box) */}
                    <div className="bg-white p-6 md:p-10 rounded-xl shadow-2xl border border-emerald-200">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:h-[400px]">
                            {/* The Upload component (left side) */}
            ...
            {/* === 3. How It Works Section (Green Block) === */}
